...
class ScanAnd {
	 ...
	 */
	public boolean isAnd(String specifiche)
		{
		/* MODELLED */
		...
		while (s.hasNext())
			...
			try {
				...
				{
				String string = specifiche + " is not and expression";
		...
		if (!scanEb.isEb(op1))
			{
			ErrorMessage errorMessage = scanEb.getErrorMessage();
			list.add(errorMessage);
			return false;
			}
		if (!scanEbt.isEbt(op2))
			{
			ErrorMessage errorMessage2 = scanEbt.getErrorMessage();
	 ...
	 */
	public And scanAnd(String specifiche)
		throws ScanException
		{
		try {
			// Eb && Ebt
			...
			while (s.hasNext()) {
				...
				ScanEb scanEb = new ScanEb(this.depth + 1);
				ScanEbt scanEbt = new ScanEbt(this.depth + 1);
				if (scanEb.isEb(op1s) && scanEbt.isEbt(op2s)) {
