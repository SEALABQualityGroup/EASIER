 ...
 *
 */
class ScanIntestBEHAV2 {

	private int depth;
	private ErrorMessage errorMessage;
	
	public ScanIntestBEHAV2(int depth) 
		{
		super();
		this.depth = depth;
	 ...
	 */
	public boolean isIntestBEHAV2(String specifiche)
		{
		/*
		...
		s.useDelimiter("\\s*\\(\\s*");
		// ie'l'identificatore per una equazione
		...
		try {
			...
			String string = specifiche + " is not header of behavioral equation";
			this.errorMessage.setErrorMessage(string);
			String string2 = "sequence of variables declarations expected";
			ErrorMessage errorMessage = new ErrorMessage(this.depth + 1);
			errorMessage.setErrorMessage(string2);
			List<ErrorMessage> list = this.errorMessage.getCauses();
			list.add(errorMessage);
	 ...
	 */
	public Header scanIntestBEHAV2(String specifiche)
		throws ScanException
		{
		try {
			/*
			...
			MyScanner s = new MyScanner(specifiche);
			s.useDelimiter("\\s*\\(\\s*");
			// ide'l'identificatore per una equazione
			// comportamentale diversa dalla prima
			String id = s.next();
			s.skip("\\s*\\(\\s*");
			ScanIdent scanIdent = new ScanIdent(this.depth + 1);
