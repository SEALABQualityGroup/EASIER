...
class NormalizeEqual {
	...
	public BooleanNorm normalizeEqual(Equal equal, String parent, DataType dataType)
		...
		if (!normalizeExpressionStrict.isErrorOccurred()) 
			...
			if (!normalizeExpressionStrict2.isErrorOccurred()) 
				...
				if ((expression5 instanceof NumberExp)
					...
					if (!(expression5 instanceof NumberExp))
						{
						String string = "Normalizing error for " + equal;
						this.errorMessage.setErrorMessage(string);
						String string2 = "Normalized expression " + expression5.toString() + " of " + expression.toString() +
							" is not number";
					...
					if (!(expression6 instanceof NumberExp))
						...
						String string = "Normalizing error for " + equal;
						this.errorMessage.setErrorMessage(string);
						String string2 = "Normalized expression " + expression6.toString() + " of " + expression2.toString() +
							" is not number";
						ErrorMessage errorMessage = new ErrorMessage(this.depth + 1);
						errorMessage.setErrorMessage(string2);
						List<ErrorMessage> list = this.errorMessage.getCauses();
    ...
    public BooleanNorm normalizeEqual(Equal equal, 
    		String parent,
    		DataType dataType,
