 ...
 *
 */
class ScanOutputInteractions {

	private int depth;
	private ErrorMessage errorMessage;
	
	public ScanOutputInteractions(int depth) 
		{
		super();
		this.depth = depth;
	 ...
	 */
	public boolean isOutputInteractions(String specifiche)
		{
		/*
		 * "OUTPUT_INTERACTIONS" <output_interactions>
		 *
		 * Poi, un'interazione di outpute'classificata essere una
		...
		if (s.hasNext("\\s*UNI\\s*(.)*")) 
			...
			try {
				...
				String string = specifiche + " is not declaration of output interactions";
				this.errorMessage.setErrorMessage(string);
				ErrorMessage errorMessage2 = new ErrorMessage(this.depth + 1);
				String string2 = "there is not declaration of uni interactions";
		...
		if (!founded)
			...
			this.errorMessage.setErrorMessage(string);
			ErrorMessage errorMessage2 = new ErrorMessage(this.depth + 1);
			String string2 = "at least one of the UNI, AND or OR element must be not empty";
			errorMessage2.setErrorMessage(string2);
			list.add(errorMessage2);
			return false;					
			}
	 ...
	 */
	public OutputInteractions scanOutputInteractions(String specifiche)
		throws ScanException
		{
		try {
			/*
			 * "OUTPUT_INTERACTIONS" <output_interactions>
			 *
			 * Poi, un'interazione di outpute'classificata essere
			 * una uni-interaction, and-interaction o or-interaction
			 * dipendente dalla moltiplicità delle comunicazioni
			 * in cui puo' essere coinvolta.
			 * Sintatticamente parlando, ogni <output_interactions> e'
			...
			ScanORinteractions scanORinteractions = new ScanORinteractions(this.depth + 1);
			if (!unis.equals(""))
				oi.setUni(scanUNIinteractions.scanUNIinteractions(unis));
			if (!ands.equals(""))
				oi.setAnd(scanANDinteractions.scanANDinteractions(ands));
			if (!ors.equals(""))
				oi.setOr(scanORinteractions.scanORinteractions(ors));
			return oi;
			} 
		catch (Exception e) 
			{
