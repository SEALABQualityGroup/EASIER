...
/**
 * Contiene dei metodi per restituire l'espressione piu' normalizzata del suo argomento
 * valutata in un determinato scope. Lo scope puo' essere aggiornato.
 * 
 * @author Acer5736Z_1
 *
 */
public class NormalizeExpressionStrict 
	{
    ...
    public NormalizeExpressionStrict(int depth)
    	{
    	super();
     ...
     * ListCons, ArrayCons, RecordCons. Se expression non puo' essere trasformato in un oggetto
     * di tipo normale si restituisce la sua forma normale.
     * dataTypee'il tipo di dato dell'espressione identificata da parent se parente'una stringa non vuota.
     * Altrimenti, dataTypee'il tipo di dato di expression. 
     * 
     * @param expression
     ...
     */
    public ExpressionNorm normalize(Expression expression,
    		String parent, 
    		DataType dataType) throws NormalizeException 
    	{
    	if (expression instanceof And)
    		...
    		booleanNorm.setOldExpression(expression);
    		...
    		{
    		ListCons listCons = (ListCons)expression.copy();
    		ListConsNorm listConsNorm = new ListConsNorm();
    		listConsNorm.setOldExpression(expression);
    		...
    		ExpressionNorm expression2 = normalizeMaggioreUguale.normalizeMaggioreUguale(maggioreUguale, parent, dataType);
    		...
    		ExpressionNorm expression2 = normalizeMoltiplicazione.normalizeMoltiplicazione(moltiplicazione, parent, dataType);
    		...
    		{
    		RecordCons recordCons = (RecordCons)expression.copy();
    		RecordConsNorm recordConsNorm = new RecordConsNorm();
    		recordConsNorm.setOldExpression(expression);
     ...
     * @param dataType - tipo dell'espressione da normalizzare. Viene utilizzato per la 
     * normalizzazione dell'espressione RecordCons
     * @param tma - scope per la normalizzazione dell'espressione.
     * 
     * @return
     * @throws NormalizeException
     */
    public ExpressionNorm normalize(Expression expression, 
    	...
    	if (expression instanceof And)
			...
			MaggioreUguale maggioreUguale = (MaggioreUguale)expression;
			NormalizeMaggioreUguale normalizeMaggioreUguale = new NormalizeMaggioreUguale(this.depth + 1);
			ExpressionNorm expression2 = normalizeMaggioreUguale.normalizeMaggioreUguale(maggioreUguale, parent, dataType, tma);
			if (!normalizeMaggioreUguale.isErrorOccurred())
	    		return expression2;
			else
				{
			...
			MinoreUguale minoreUguale = (MinoreUguale)expression;
			NormalizeMinoreUguale normalizeMinoreUguale = new NormalizeMinoreUguale(this.depth + 1);
			ExpressionNorm expression2 = normalizeMinoreUguale.normalizeMinoreUguale(minoreUguale, parent, dataType, tma);
			if (!normalizeMinoreUguale.isErrorOccurred())
	    		return expression2;
			else
				{
			...
			Moltiplicazione moltiplicazione = (Moltiplicazione)expression;
			NormalizeMoltiplicazione normalizeMoltiplicazione = new NormalizeMoltiplicazione(this.depth + 1);
			ExpressionNorm expression2 = normalizeMoltiplicazione.normalizeMoltiplicazione(moltiplicazione, parent, dataType, tma);
			if (!normalizeMoltiplicazione.isErrorOccurred())
	    		return expression2;
			else
				{
